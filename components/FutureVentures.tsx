...
function VentureCard({ venture, delay }: { venture: Venture; delay: number }) {
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute left-0 top-0 h-4 w-4 border-l border-t border-navy/20 transition-all duration-500 ease-out group-hover:h-8 group-hover:w-8 group-hover:border-green sm:h-5 sm:w-5 sm:group-hover:h-9 sm:group-hover:w-9"
      />
      <span
        aria-hidden
      ...
      <div className="flex flex-1 flex-col border border-navy/10 p-6 transition-colors duration-500 group-hover:border-navy/20 sm:p-7 lg:p-8">
        <p className="text-[13px] tracking-wide text-muted-foreground">
          {venture.location}
        </p>

        <h3 className="mt-3 max-w-[22ch] text-balance font-display text-2xl leading-[1.15] text-navy sm:text-[1.75rem]">
          {venture.title}
        </h3>
        ...
        <div className="mt-4 flex-1 sm:mt-5">
          ...
          {venture.description.length > 140 && (
            <button
              ...
              className="mt-3 rounded-sm text-[13px] text-navy underline decoration-navy/25 decoration-1 underline-offset-4 transition-colors duration-200 hover:decoration-green focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green/50 focus-visible:ring-offset-2 focus-visible:ring-offset-sand-50"
            >
